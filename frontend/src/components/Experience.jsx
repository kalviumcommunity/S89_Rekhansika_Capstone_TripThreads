...
const experienceData = [
  {
    ...
    title: 'Maldives',
    description:
      'The Maldives is one of the world’s most beautiful destinations, famous for its pristine beaches and crystal clear waters.',
    imageUrl:
      'https://i.pinimg.com/736x/f0/cc/66/f0cc66b5884386036adc2cba8a669d99.jpg',
  },
  {
    id: 2,
    title: 'Paris',
    description:
      'Paris, the city of lights, is known for its art, fashion, and culture. A must-visit for every traveler.The city is also a center for culture, history, and gastronomy.',
    imageUrl:
      'https://i.pinimg.com/736x/30/99/d3/3099d36c38c27acbd79ae5cee41dd51c.jpg'
  }
];
...
const Experience = () => {
  ...
  const [experiences, setExperiences] = useState(getInitialExperiences);
  useEffect(() => {
...
const handleDelete = (id) => {
            ...
            <div key={experience.id} className="card experience-card">
              ...
              <div className="icons">
                ...
                <img
                  ...
                  alt="location"
                  onClick={() => handleLocationPicker(experience.id)}
                  className="icon"
                />
                <img
                  src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRzspEkAFUukBRCWMZsyRDn48nu1pjEsvq27Q&s"
                  alt="calendar"
                  onClick={() => handleDatePicker(experience.id)}
                  className="icon"
                />
                {datePickerOpen[experience.id] && (
                  <DatePicker
                    selected={selectedDate[experience.id] || null}
                    onChange={(date) => handleDateChange(experience.id, date)}
                    inline
                  />
                )}
                {locationPickerOpen[experience.id] && (
                  <div className="location-picker">
                    <Select
                      options={locationOptions}
                      value={selectedLocation[experience.id] || null}
  ...
  <button className="delete-btn" onClick={() => handleDelete(experience.id)}>Delete</button>
</div>
              </div>
            </div>
          ))}
